...
## Cambios Aplicados En Restaurantes

- Se corrigio `GET /api/restaurantes/:restaurante_id/pedidos` para ordenar por `fecha_creacion`, evitando el error por columna inexistente `fecha_pedido` en el esquema actualizado.
- Se ajusto la creacion de historial de pedidos para guardar `estado_nombre`, requerido por `historial_estados_pedido`.
- Se agrego `GET /api/restaurantes/:restaurante_id/pedidos/:pedido_id/logistica` con payload logistico para Logistica.
- Se agrego `PATCH /api/restaurantes/:restaurante_id/pedidos/:pedido_id/logistica-status` para recibir eventos de Logistica.
- Se mantuvo compatible `PUT /api/restaurantes/:restaurante_id/pedidos/:pedido_id/estado` y ahora acepta `metadata` de Logistica.
